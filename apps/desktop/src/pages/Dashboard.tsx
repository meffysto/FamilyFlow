    ...
    <div className="page">
      ...
      <div className="dashboard-grid">
          ...
          {pendingTasks.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              {pendingTasks.slice(0, 8).map((task, i) => (
                <div
                  key={`${task.sourceFile}-${task.lineIndex}-${i}`}
                  style={{
                    display: 'flex',
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 2, flexWrap: 'wrap' }}>
                      {task.dueDate && (
                        <Badge
                          variant={task.dueDate === todayStr ? 'warning' : task.dueDate < todayStr ? 'error' : 'default'}
                          size="sm"
                        >
                          📅 {formatShortDate(task.dueDate)}
                        </Badge>
        ...
        <GlassCard
          title="Rendez-vous"
          icon="📆"
          count={todayRdvs.length + upcomingRdvs.length}
        ...
        {nextAnniversaries.length > 0 && (
          <GlassCard
            ...
            linkText="Voir tout →"
            onLinkClick={() => navigate('/birthdays')}
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {nextAnniversaries.map((a, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0' }}>
                  ...
                  <Badge
                    variant={a.daysUntil === 0 ? 'success' : a.daysUntil <= 7 ? 'warning' : 'default'}
                    size="sm"
                  >
