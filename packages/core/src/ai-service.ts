...
export interface AIVaultContext {
  tasks: Task[];
  rdvs: RDV[];
  stock: StockItem[];
  meals: MealItem[];
 ...
 */
function filterByProfile(ctx: AIVaultContext): AIVaultContext {
...
function buildVaultSummary(ctx: AIVaultContext): VaultSummary {
...
function prepareAnonymized(
  ...
  const filtered = filterByProfile(ctx as AIVaultContext);
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (process.env.NODE_ENV !== 'production') console.log('🔒 [ANON] Mapping: ', anonMap.forward.size, 'entrées');
  }
...
export async function askVault(
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (process.env.NODE_ENV !== 'production') console.log('🔒 [ANON] Question envoyée (anonymisée)');
  }
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (process.env.NODE_ENV !== 'production') console.log('🔓 [ANON] Réponse reçue, longueur:', deanoText.length);
  }
...
- Si l'image n'est pas un ticket de caisse, réponds {"store":"","date":"","items":[],"total":0}`;
  ...
  if (resp.error) {
    if (process.env.NODE_ENV !== 'production') console.log('🧾 [RECEIPT] Erreur Vision:', resp.error);
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') console.log('🧾 [RECEIPT] Réponse brute (100 premiers chars):', cleaned.slice(0, 100));
    ...
    if (process.env.NODE_ENV !== 'production') console.log('🧾 [RECEIPT] Erreur parsing JSON:', e, '\nRéponse brute:', resp.text);
