 ...
 *
 * NOTE: awardTaskCompletion and lootProgress now accept a GamificationConfig
 * parameter instead of calling loadGamiConfig (which uses expo-secure-store).
 * Pass getCachedGamiConfig() from the app layer.
 */
 ...
 *
 * @param config - Pass getCachedGamiConfig() from app layer (defaults to DEFAULT_GAMI_CONFIG)
 */
export function awardTaskCompletion(
  profile: Profile,
  taskNote: string,
  config: GamificationConfig = getCachedGamiConfig(),
