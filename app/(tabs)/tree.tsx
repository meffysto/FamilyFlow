...
export default function TreeScreen() {
  ...
  // Réinitialise l'état des sections du picker à chaque ouverture
  useEffect(() => {
    if (!showSeedPicker) return;
    const rareSeeds = profile?.farmRareSeeds ?? {};
    const hasRare = CROP_CATALOG.some(c => c.dropOnly && !c.epic && (rareSeeds[c.id] ?? 0) > 0);
    const hasEpic = CROP_CATALOG.some(c => c.epic && (rareSeeds[c.id] ?? 0) > 0);
    setRareExpanded(hasRare);
    setEpicExpanded(hasEpic);
    setExpeditionExpanded(false);
  }, [showSeedPicker]);
    ...
    if (crop && crop.currentStage >= 4) {
      ...
      harvest(profile.id, crop.plotIndex).then((result) => {
        if (result) {
          ...
          // Animation graine rare : épique après la fin du HarvestEventOverlay (1800ms)
          if (result.seedDrop) {
            const seedDelay = result.seedDrop.epic ? 2200 : 1500;
            setTimeout(() => setSeedDropEvent(result.seedDrop), seedDelay);
          }
  ...
  return (
    <View ref={screenRef} style={[styles.safe, { backgroundColor: colors.bg }]} onTouchStart={pokeActivity}>
      ...
      >
        ...
        {/* Modal choix de graine — pageSheet cozy (auvent + parchemin) */}
        <Modal
        ...
        >
          <View style={styles.seedSheetContainer}>
            ...
            <View style={styles.seedAwning}>
              ...
              <View style={styles.seedAwningScallop}>
                {Array.from({ length: 12 }).map((_, i) => (
                  <View key={i} style={styles.seedAwningScallopDot} />
                ))}
              </View>
            </View>

            <View style={styles.seedParchment}>
              {/* Bouton fermer bois */}
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <Text style={styles.seedCloseBtnText}>{'✕'}</Text>
              </TouchableOpacity>

              {/* Handle + feuilles */}
              <View style={styles.seedHandleRow}>
                <View style={styles.seedHandleBadge}>
                  <View style={styles.seedHandle} />
                  <View style={styles.seedHandleCoinsRow}>
                    <Text style={styles.seedHandleCoins}>{profile?.coins ?? 0}</Text>
                    <Text style={styles.seedHandleCoinsEmoji}>🍃</Text>
                  </View>
                </View>
              </View>
              ...
              <ScrollView
                ...
                showsVerticalScrollIndicator={false}
              >
                {/* ── Graines normales (achetables) — groupées par stade ── */}
                <View style={styles.seedSectionHeader}>
                  <Text style={styles.seedSectionTitle}>
                    {t('farm.regularSeeds')}
                  </Text>
                </View>
                {(() => {
                  const stageOrder = ['graine', 'pousse', 'arbuste', 'arbre', 'majestueux', 'legendaire'] as const;
                  const regular = CROP_CATALOG.filter(c => !c.dropOnly);
                  const grouped = stageOrder
                    .map(stage => ({
                      stage,
                      crops: regular
                        .filter(c => c.minTreeStage === stage)
                        .sort((a, b) => a.cost - b.cost),
                    }))
                    .filter(g => g.crops.length > 0);
                  return grouped.map(({ stage, crops }) => (
                    <React.Fragment key={`stage-${stage}`}>
                      <View style={styles.seedStageHeader}>
                        <Text style={styles.seedStageTitle}>
                          {t(`mascot.stages.${stage}`)}
                        </Text>
                      </View>
                      {crops.map(crop => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.seedRow,
                        isSeasonal && { borderColor: '#F59E0B', borderWidth: 2 },
                    ...
                    >
                      ...
                      {unlocked ? (
                        <View style={styles.seedRowInfo}>
                          ...
                          <Text style={styles.seedRowDesc} numberOfLines={2}>
                          ...
                          <View style={styles.seedRowStats}>
                            <Text style={styles.seedRowStat}>
                              ...
                              {isSeasonal && (
                                <Text style={{ color: '#B45309', fontWeight: FontWeight.semibold }}>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <TouchableOpacity
                        onPress={() => setEpicExpanded(v => !v)}
                        activeOpacity={0.7}
                        style={[styles.seedSectionHeader, { marginTop: Spacing.md, borderBottomColor: EPIC_COLOR }]}
                      >
                        <Text style={[styles.seedSectionTitle, { color: EPIC_COLOR }]}>
                          {t('farm.epicSeedsTitle')} ⭐
                        </Text>
                        <Text style={[styles.seedSectionChevron, { color: EPIC_COLOR }]}>
                          {epicExpanded ? '▼' : '▶'}
                        </Text>
                      </TouchableOpacity>
                      {epicExpanded && epicSeeds.map(crop => {
                ...
                {(() => {
                  ...
                  const getDropHint = (cropId: string): string => {
                    const rule = RARE_SEED_DROP_RULES.find(r => r.seedId === cropId);
                  ...
                  const renderLockedRareRow = (crop: typeof CROP_CATALOG[number], isExpeditionOnly: boolean) => {
                    const effectiveTasksPerStage = Math.max(1, crop.tasksPerStage - (techBonuses?.tasksPerStageReduction ?? 0));
                    const totalTasks = effectiveTasksPerStage * 4;
                    return (
                      <View
                        ...
                        style={[
                          styles.seedRow,
                          isExpeditionOnly
                            ? { borderColor: 'rgba(59,130,246,0.4)', borderWidth: 2, opacity: 0.75 }
                            : { opacity: 0.5 },
                        ]}
                      >
                        <View>
                        ...
                        <View style={styles.seedRowInfo}>
                          ...
                          <Text style={[styles.seedRowDesc, isExpeditionOnly && { color: '#2563EB' }]} numberOfLines={2}>
                  ...
                  return (
                    <>
                      <TouchableOpacity
                        onPress={() => setRareExpanded(v => !v)}
                        activeOpacity={0.7}
                        style={[styles.seedSectionHeader, { marginTop: Spacing.md }]}
                      >
                        <Text style={[styles.seedSectionTitle, { color: primary }]}>
                          {t('farm.rareSeedsTitle')} ✨
                        </Text>
                        <Text style={[styles.seedSectionChevron, { color: primary }]}>
                          {rareExpanded ? '▼' : '▶'}
                        </Text>
                      </TouchableOpacity>
                      {rareExpanded && (
                        <>
                          ...
                          {/* Graines non possedees — drop récolte */}
                          {lockedHarvest.length > 0 && (
                            <View style={styles.seedStageHeader}>
                              <Text style={styles.seedStageTitle}>
                                {t('farm.rareHarvestGroup')}
                              </Text>
                            </View>
                          )}
                          {lockedHarvest.map(crop => renderLockedRareRow(crop, false))}
                          {/* Graines expédition — sous-section collapsible */}
                          {lockedExpedition.length > 0 && (
                            <TouchableOpacity
                              onPress={() => setExpeditionExpanded(v => !v)}
                              activeOpacity={0.7}
                              style={styles.seedStageHeader}
                            >
                              <Text style={styles.seedStageTitle}>
                                {t('farm.rareExpeditionGroup')}
                              </Text>
                              <Text style={[styles.seedSectionChevron, { color: colors.textFaint, fontSize: 10 }]}>
                                {expeditionExpanded ? '▼' : '▶'}
                              </Text>
                            </TouchableOpacity>
                          )}
                          {expeditionExpanded && lockedExpedition.map(crop => renderLockedRareRow(crop, true))}
                        </>
                      )}
                    </>
                  );
...
const makeStyles = (farm: FarmPalette) => StyleSheet.create({
  ...
  cozyRowPlaced: {
    ...
    backgroundColor: farm.greenBtn + '15',
  },
  cozyRowSprite: {
    width: 48,
    height: 48,
