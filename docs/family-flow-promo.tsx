...
// ── Font (inlined as system font fallback since @remotion/google-fonts not available in sandbox) ──
...
// ── Helpers ──
function spr(frame: number, fps: number, cfg = { damping: 200 }, delay = 0) {
...
function AppIcon({ size = 120, scale = 1 }: { size?: number; scale?: number }) {
  return (
    ...
    >
      <span style={{ color: "#fff", fontSize: size * 0.38, fontWeight: 800, letterSpacing: -1 }}>
        FF
      </span>
    </div>
  );
  ...
  return (
    ...
    >
      <div style={{ fontFamily: FONT, fontSize: 48, fontWeight: 800, color: C.black, textAlign: "center", letterSpacing: -2, lineHeight: 1.15, minHeight: 58 }}>
        {line1}
        {frame >= 60 && (
          <span style={{ transform: `scale(${dashScale})`, display: "inline-block", margin: "0 6px", color: C.purple }}>—</span>
        )}
      </div>
...
function SlideLeft({ progress, children }: { progress: number; children: React.ReactNode }) {
  return (
    <div style={{ transform: `translateX(${interpolate(progress, [0, 1], [0, -1080])}px)`, width: "100%", height: "100%" }}>
      {children}
    </div>
  );
}
...
// ── MAIN ──
export default function FamilyFlowPromo() {
  const frame = useCurrentFrame();
  const { fps, durationInFrames } = useVideoConfig();

  // Scene timings (start frames, 15-frame overlaps via negative offset)
  const T = 15; // transition overlap
  const scenes = [
  ...
  function sceneFrame(i: number) {
    return frame - scenes[i].start;
  }

  function sceneOpacity(i: number) {
    const { start, end } = scenes[i];
    if (frame < start) return 0;
    if (frame > end) return 0;
    const fadeIn = interpolate(frame, [start, start + T], [0, 1], { extrapolateRight: "clamp" });
    const fadeOut = interpolate(frame, [end - T, end], [1, 0], { extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ background: C.white, overflow: "hidden" }}>
      {scenes.map((s, i) => {
        const op = sceneOpacity(i);
        if (op === 0) return null;
        const localFrame = sceneFrame(i);

        return (
          <AbsoluteFill key={i} style={{ opacity: op }}>
            <Sequence from={0} durationInFrames={s.end - s.start}>
              {i === 0 && <SceneLogo />}
              {i === 1 && <SceneSlogan1 />}
              {i === 2 && <SceneDashboard />}
              {i === 3 && <SceneCalendar />}
              {i === 4 && <SceneMeals />}
              {i === 5 && <SceneGamification />}
              {i === 6 && <SceneSloganClimax />}
              {i === 7 && <SceneClosing />}
            </Sequence>
          </AbsoluteFill>
        );
